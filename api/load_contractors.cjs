...
// Flip to false after we confirm column offsets from the test dump
...
(async () => {
  for (const src of SOURCES) {
    ...
    // ── TEST MODE: dump first 3 rows as indexed columns, then stop ──
    if (TEST_MODE) {
      ...
      console.log("\n  TEST_MODE on — no data written. Confirm offsets, then set TEST_MODE=false.");
      continue;
    }

    // ── LOAD MODE: map confirmed offsets → upsert ──
    // NOTE: indices below are PLACEHOLDERS — set from the test dump.
    const I = {
      license_type: 0, license_number: 1, license_subtype: 2, license_exp: 3,
      name: 4, business_name: 5,
      mail_city: 8, mail_state: 9, mail_zip: 10,
      phone: 12,
      business_city: 16, business_county: 18, business_zip: 17, business_phone: 19,
    };
    const hasHeader = isNaN(parseInt(parseCsvLine(lines[0])[I.license_number])) ||
                      /license/i.test(lines[0]);
    ...
    for (let r = start; r < lines.length; r++) {
      ...
      const county = c[I.business_county] || "";
      batch.push({
        app_id: "flowstate",
        trade: src.trade,
        ...
        name: c[I.name] || null,
        business_name: c[I.business_name] || null,
        phone: c[I.phone] || null,
        business_phone: c[I.business_phone] || null,
        mailing_city: c[I.mail_city] || null,
        mailing_state: c[I.mail_state] || null,
        mailing_zip: c[I.mail_zip] || null,
        business_city: c[I.business_city] || null,
